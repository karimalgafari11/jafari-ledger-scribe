...
const Reports = () => {
  ...
  return <div className="h-screen w-full overflow-y-auto bg-gray-50">
      <div className="sticky top-0 z-10 bg-white shadow-sm w-full">
        <Header title="التقارير" showBack={true}>
          <div className="flex items-center gap-4 rtl">
            <FilterPopover activeCategory={activeCategory} onFilterChange={handleFilter} />
            ...
            <Button onClick={handleNewReport} className="bg-teal hover:bg-teal-dark text-white">
      ...
      <main className="p-6 w-full bg-cyan-300 px-0 py-[10px]">
        <Tabs defaultValue="list" className="mb-6 w-full">
          <div className="flex justify-between items-center mb-4">
            ...
            {filteredReports.length > 0 && <ReportExportOptions reportTitle="قائمة التقارير" />}
          </div>

          <TabsContent value="list" className="w-full bg-cyan-50 mx-0 px-0">
            {/* Search Bar */}
            <div className="mb-6 max-w-md mx-auto">
              <div className="relative rtl">
                <Input type="text" placeholder="ابحث عن تقرير..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full" />
              </div>
            </div>
          ...
          <TabsContent value="search" className="w-full">
            <AdvancedReportFilters dateRange={dateRange} setDateRange={setDateRange} category={selectedCategory} setCategory={setSelectedCategory} searchQuery={searchQuery} setSearchQuery={setSearchQuery} onApplyFilters={handleApplyFilters} onResetFilters={handleResetFilters} />

            <div className="bg-white p-6 rounded-md shadow mb-6 w-full">
              <h3 className="text-lg font-medium mb-4 rtl">نتائج البحث</h3>
              
              {filteredReports.length > 0 ? <div className="grid grid-cols-1 gap-4 w-full">
                  {filteredReports.map(report => <div key={report.id} className="p-4 border rounded-md hover:bg-gray-50 transition-colors flex justify-between items-center rtl w-full">
                ...
                </div> : <div className="text-center py-8 bg-gray-50 rounded-md border border-dashed w-full">
