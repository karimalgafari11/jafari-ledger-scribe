...
export const ProductShareDialog: React.FC<ProductShareDialogProps> = ({
  open,
  onOpenChange,
  data,
  selectedProducts
  ...
  const [message, setMessage] = useState(`بيانات المنتجات - ${new Date().toLocaleDateString('ar-SA')}`);
  ...
  const productsToShare = selectedProducts.length > 0
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>مشاركة بيانات المنتجات</DialogTitle>
          <DialogDescription>
            مشاركة {productsToShare.length} منتج 
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="space-y-2">
                ...
                <div className="flex">
                  <Input 
                    id="link" 
                    readOnly
                    value="https://example.com/shared/products/xyz123"
                    dir="ltr"
                  />
                  <Button 
                    className="mr-2"
