  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="space-y-4">
        <div>
          ...
          <Input id="invoiceNumber" value={invoice.invoiceNumber} readOnly className="bg-muted" />
        </div>

        <div>
          <label htmlFor="date" className="block text-sm font-medium mb-1">تاريخ الفاتورة</label>
          <Input id="date" type="date" value={invoice.date ? format(new Date(invoice.date), 'yyyy-MM-dd') : format(new Date(), 'yyyy-MM-dd')} onChange={e => onFieldChange('date', e.target.value)} />
        </div>

        <div>
          <label htmlFor="warehouse" className="block text-sm font-medium mb-1">المخزن</label>
          <Select value={invoice.warehouseId || ""} onValueChange={value => {
      ...
      <div className="space-y-4 py-0">
        <div>
          ...
          <Select value={invoice.customerId || ""} onValueChange={value => {
        ...
        <div>
          ...
          <RadioGroup defaultValue={invoice.paymentMethod || "cash"} className="flex space-x-4 rtl space-x-reverse" onValueChange={value => onFieldChange('paymentMethod', value)}>
